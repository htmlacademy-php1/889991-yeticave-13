...
function get_query_lot ($id_lot) {
    return "SELECT lots.title, lots.start_price, lots.img, lots.date_finish, lots.lot_description, lots.step, categories.name_category FROM lots
    JOIN categories ON lots.category_id=categories.id
...
function get_bets_history ($con, $id_lot) {
    ...
    return $error;
        ...
        $sql = "SELECT bets.date_bet, bets.price_bet, lots.title, users.user_name
        FROM bets
